...
def test_add_subject(session):
    subject_table = Table('subject', MetaData(),
    ...
    result = session.query(subject_table).filter_by(subject_id=new_subject['subject_id']).first()
    assert result.subject_title == new_subject['subject_title']

    delete_stmt = subject_table.delete().where(subject_table.c.subject_id == new_subject['subject_id'])
...
def test_update_student(session):
    ...
    update_stmt = student_table.update().where(student_table.c.user_id == update_data['user_id'])\
                                        .values(level=update_data['level'])
    session.execute(update_stmt)
    session.commit()

    result = session.query(student_table).filter_by(user_id=update_data['user_id']).first()
    assert result.level == update_data['level']

    rollback_data = {'level': 'Upper-Intermediate'}
    rollback_stmt = student_table.update()\
...
def test_delete_teacher(session):
    teacher_table = Table('teacher', MetaData(), 
                          Column('teacher_id', Integer, primary_key=True), 
                          Column('email', String), 
                          Column('group_id', Integer)) 
    
    delete_data = {"teacher_id": 29971}
    delete_stmt = teacher_table.delete()\
                               .where(teacher_table.c.teacher_id == delete_data["teacher_id"])
    session.execute(delete_stmt)
    session.commit()

    result = session.query(teacher_table).filter_by(teacher_id=delete_data['teacher_id']).all()
    assert len(result) == 0
